...
import { ProcessedElement, DocumentSection, ProcessingOptions } from '../types/index.js';

const createLogger = () => (level: string, message: string, extra: Record<string, any> = {}) => {
...
const formatContent = (elements: ProcessedElement[]): string => {
  ...
  if (sections.titles.length > 0) docTexts.push('# ' + sections.titles.join('\n# '));
  if (sections.headers.length > 0) docTexts.push('\n## ' + sections.headers.join('\n## '));
  if (sections.paragraphs.length > 0) docTexts.push('\n' + sections.paragraphs.join('\n\n'));
  if (sections.lists.length > 0) docTexts.push('\n' + sections.lists.join('\n'));
  if (sections.tables.length > 0) docTexts.push('\n### Tables:\n' + sections.tables.join('\n\n'));
  if (sections.other.length > 0) docTexts.push('\n### Additional Content:\n' + sections.other.join('\n'));

  return docTexts.join('\n');
};

const processDocument = async (filepath: string, options?: ProcessingOptions): Promise<string> => {
  ...
  try {
    ...
    const formattedContent = formatContent(res as ProcessedElement[]);
