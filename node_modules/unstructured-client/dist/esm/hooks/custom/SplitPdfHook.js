...
import { getOptimalSplitSize, getSplitPdfAllowFailed, getSplitPdfConcurrencyLevel, getStartingPageNumber, getSplitPdfPageRange, loadPdf, prepareRequestBody, prepareRequestHeaders, prepareResponseBody, prepareResponseHeaders, splitPdf, stringToBoolean, } from "./utils/index.js";
import { HTTPClientExtension, generateGuid, MIN_PAGES_PER_THREAD, PARTITION_FORM_FILES_KEY, PARTITION_FORM_SPLIT_PDF_PAGE_KEY, } from "./common.js";
...
export class SplitPdfHook {
    ...
    async beforeRequest(hookCtx, request) {
        ...
        const splitPdfPage = stringToBoolean(formData.get(PARTITION_FORM_SPLIT_PDF_PAGE_KEY) ?? "false");
        const file = formData.get(PARTITION_FORM_FILES_KEY);
        if (!splitPdfPage) {
            return request;
        }
        if (!this.client) {
        ...
        for (const { content, startPage } of splits) {
            ...
            const body = await prepareRequestBody(formData, content, file.name, firstPageNumber);
        ...
        this.partitionRequests[operationID] = async.parallelLimit(requests.map((req, pageIndex) => async () => {
